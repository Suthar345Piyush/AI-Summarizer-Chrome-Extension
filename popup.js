document.getElementById("summarize").addEventListener("click" , () => {
 ...
 async function getGeminiSummary(text , summaryType , apiKey){
    ...
    switch (summaryType){
     ...
     case  "bullets":
       prompt = `Summarize the following article  in 5-7 key points. Format each point as a line starting with "- " (dash followed by a space). Do not use astericks or other bullet symbols , only use the dash. Keep each point concise and focused on a single key insight from the article:\n\n${truncateText}`;
       break;
